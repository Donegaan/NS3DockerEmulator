...
def create():
    ...
    r_code = subprocess.call(
        "cd ns3 && bash update.sh tap-mesh-with-wifi.cc", shell=True)
    if r_code != 0:
    ...
    for x in range(0, numberOfProdNodes):  # Run containers for each producer
        ...
        acc_status += subprocess.call(
            "docker run --privileged -d -t -i --net=none %s --name %s %s" % (
...
def run_emu():
    ...
    for x in range(0, numberOfNodes):
        ...
        acc_status += subprocess.call("docker exec -d %s python3 create_stream.py " %  # Create stream
                                      (nameList[x]), shell=True)

        # acc_status += subprocess.call("docker exec -d %s python3 publish_objects.py " %  # Connect nodes to created blockchain
        #                               (nameList[x]), shell=True)

        # acc_status += subprocess.call("docker exec -d %s python3 request_objects.py " %  # Connect nodes to created blockchain
        #                               (nameList[x]), shell=True)
